...
class UsageSession:
    ...
    Thread-safe: add() is guarded by a lock so a task may fan its provider
    calls out across threads (this app runs chunk calls concurrently) and still
    accumulate into a single session safely.
    """
    ...
    def add(self, model, tokens_in=0, tokens_out=0, cost_inr=0.0):
        with self._lock:
...
def gemini_generate(
    ...
    *,
    ...
    """Call Gemini THROUGH the proxy. The proxy holds GEMINI_API_KEY, calls
